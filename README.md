...
## 6. Что было сделано для рефакторинга

* Добавлен пакет [`spatie/laravel-json-api-paginate`](https://github.com/spatie/laravel-json-api-paginate),
  `jsonPaginate()` используется в `JokeController::index()` и `VisitController::index()`.
* Контроллеры стали **тонкими**: только адаптеры HTTP ↔ сервис ↔ Resource.
* Бизнес‑логика вынесена в `App\Services\Jokes` и `App\Services\Visits`.
* Валидация — через **Form Request** (`StoreVisitRequest`), DTO `VisitData`.
* Сериализация — через **Api Resource** (`JokeResource`, `VisitResource`).
* `GeoIp` перестал быть статикой — теперь это инжектируемый сервис с
  настраиваемыми `timeout`/`ttl`/`http-factory`/`cache`.
* `UserAgentParser` оставлен статикой как чистая функция (без зависимостей).

---

## 7. Деплой на хостинг (для проверки тестового)

Самый быстрый способ выложить проверяющему:

### Вариант A. Railway (рекомендуется)

1. `railway init` → `railway link` к новому проекту.
2. В `railway.json` указать корневой Dockerfile (`docker/Dockerfile`)
   или подключить «PHP via Nixpacks» (Railway сам распознает Laravel).
3. Переменные окружения:
   ```
   APP_KEY=$(php artisan key:generate --show)
   APP_ENV=production
   APP_DEBUG=false
   APP_URL=https://your.up.railway.app
   DB_CONNECTION=sqlite
   DB_DATABASE=/data/database.sqlite      # volume в /data
   ```
4. На Railway добавить volume `/data`, чтобы SQLite пережил рестарты.
5. Cron‑команда: Railway → Settings → Cron → `php artisan schedule:run`.

### Вариант B. Любой VPS (DigitalOcean, Timeweb, Selectel, etc.)

```bash
git clone <repo> && cd amopoint
docker compose up -d --build
# системный cron на хосте:
* * * * * docker compose exec -T app php artisan schedule:run >> /var/log/amopoint.log 2>&1
```

И настроить nginx/Caddy перед контейнером для HTTPS.

### Вариант C. Render / Fly.io

В обоих случаях используем `docker/Dockerfile`. Cron на Render — через
секцию "Cron Jobs", на Fly.io — через `[processes.scheduler]` в `fly.toml`,
указывая `php artisan schedule:work`.

> Из‑за того, что задание оценивается локально, готовый Docker Compose
> покрывает 100% функционала «локальной проверки». Деплой на публичный
> хостинг — конфигурационная упаковка, скриптов специально не приложено,
> чтобы не плодить чувствительные креды.

---

## 8. Чек‑лист проверки (быстрый)

- [ ] `docker compose up -d` — стек поднялся;
- [ ] `docker compose exec app php artisan jokes:fetch` → `Stored joke #N`;
- [ ] `docker compose exec app php artisan schedule:list` → `*/5 * * * *  php artisan jokes:fetch`;
- [ ] `curl 'http://localhost:8000/api/jokes?page[size]=5'` → валидный JSON:API ответ;
- [ ] `curl -X POST http://localhost:8000/api/track -H 'Content-Type: application/json' -d '{"visitor_id":"x"}'` → `{"ok":true,"id":...}`;
- [ ] `http://localhost:8000/login` → форма; вход `admin@amopoint.local`/`secret123` → `/dashboard` с графиками;
- [ ] `snippet/testlist.html` — меняем «Тип», нужные поля скрываются;
- [ ] `snippet/testlist-filter.snippet.js` — копируется в консоль на любой похожей странице.
